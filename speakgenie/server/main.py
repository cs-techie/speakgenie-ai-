...
# === Mount static folder (serve media files)
...
async def tts(
    ...
    - For simplicity & reliability on Windows, we return MP3 only.
    """
    t = (text or "").strip()
    if not t:
...
def sample_audio():
    """
    Generates a sample audio file and serves it via /media/tts/.
    """
    out_path = TTS_DIR / "test.mp3"
    gTTS("Hello! My name is Sam. How are you today?", lang="en").save(out_path)
    return FileResponse(out_path, media_type="audio/mpeg", filename="test.mp3")
